...
void main(void)
{
	...
	init_config();
	init_config_ssd();
	unsigned char node_id[5] = {'\0'};
	unsigned char receive_array[5] = {'\0'};

	static short i = 0, j = 0, select = 0, option = 1;
	static short k = 0, l = 0, temp = 0, re_f = 0;
	...
	*/ssd[0] = '\0';
	...
	while (1)
	{
		...
		if(flag)
		{
			...
			else if(temp == 1)
			{
				...
				if (key == SWITCH4)
				{
					...
					can_payload[D3] = read_internal_eeprom(0x01) + 48; //i+48;//0x35;//
					can_payload[D2] = read_internal_eeprom(0x02) + 48;//j+48;//0x34;//read_internal_eeprom(0x02) + 30;
					can_payload[D1] = read_internal_eeprom(0x03) + 48;
					can_payload[D0] = read_internal_eeprom(0x04) + 48;
					can_payload[D7] = read_internal_eeprom(0x05) + 48;
					can_payload[D6] = read_internal_eeprom(0x06) + 48;
					can_payload[D5] = read_internal_eeprom(0x07) + 48;
					can_payload[D4] = read_internal_eeprom(0x08) + 48;	
		...
		else
		{
			...
			temp = 0;	
			node_id[3] = read_internal_eeprom(0x01) + 48;
			node_id[2] = read_internal_eeprom(0x02) + 48;
			node_id[1] = read_internal_eeprom(0x03) + 48;
			node_id[0] = read_internal_eeprom(0x04) + 48;

			if (can_receive())
			{
				receive_array[0] = can_payload[D0];
				receive_array[1] = can_payload[D1];
				receive_array[2] = can_payload[D2];
				receive_array[3] = can_payload[D3];
				
				
				if(strncmp(node_id, receive_array,4) == 0)
				{
					ssd[0] = num[can_payload[D4] - 48];
					ssd[1] = num[can_payload[D5] - 48];
					ssd[2] = num[can_payload[D6] - 48];
					ssd[3] = num[can_payload[D7] - 48];
